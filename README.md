...
8.18 为 WebAdmin 配置增加 `#/snapshots` 时间轴：手动保存点、关键 WebAdmin 写操作前自动快照、与上一保存点的资源级 diff、rollback dry-run 预览和确认后回滚。

核心实现：

- `WebAdminSnapshotStore` 保存 world-scoped manifest/package：`tzz/webadmin/snapshots/manifest.json` 与 `tzz/webadmin/snapshots/data/<snapshotId>.json`。
- `SnapshotManifest` / `SnapshotRecord` / `SnapshotPackage` / `SnapshotResource` / `SnapshotDiffSummary` / `RollbackPlan` 组成 8.18 schema。
- 手动保存点支持名称、备注、标签。
...
- 自动快照默认保留最近 200 个；manual 和 `pre_rollback` 不自动删除。

边界：

- 不纳入 `logs/`、`.codex/`、`build/`、`run/`、`.gradle/`、`node_modules/`。
- 不备份 runtime history、Condition debugger history、Timer active runtime state、Join pending runtime state、玩家实时背包或世界实体。
- 当前 `state_variables.json` 是配置/值混合文件；8.18 将其作为 WebAdmin 可见 store 纳入 allowlist，但 rollback 对已有 StateVariable 保留当前值，只恢复定义结构，避免把运行态值回拨成旧快照值。`signal_devices.json` 也会在回滚时保留仍存在设备的常见 `last*` runtime 字段。它不是 live inventory 或世界状态备份。
- 不做 Git branch / merge / rebase，不做 full Logic Chain Editor、Scratch editor、if / else runtime、GameController / MissionSystem / PhaseController，不新增 ActionType / ConditionNodeType。
...
- 新频道 / 新逻辑链：连接模式可添加 draft channel endpoint；频道端点 combobox 与 Signal Join 和 Timer 共享“选择已有频道 / 创建新下游频道端点”的输出能力。频道字段使用 dark combobox，显示已有 channel 的 displayName + channel id，也允许输入新 channel id。保存时只使用真实 channel id；新 channel display metadata / root channel metadata 写入 WebAdmin metadata，不保存假图节点；端点连接失败、取消或替换会清理不再被连线引用的 metadata，后端也会拒绝 unreferenced metadata，并且只在 typed 写入成功后尝试保存频道 metadata。Action append payload 不允许携带 draft edges，Signal action append 的频道 metadata 只能引用 action value，避免伪造边保存孤立频道。Logic Chain 列表提供“新建逻辑链”，让独立链出现在列表中，不强制挂到当前 root。
