...
public final class RegionTitleOverlay {
    ...
    public static void render(DrawContext context) {
        ...
        // Tech-themed region title: angular box with accent
        int bgAlpha = Math.round(alpha * 0.75F);
        int lineAlpha = Math.round(alpha * 0.85F);
        int techBg = (bgAlpha << 24) | (lightMode ? 0xE8EDF4 : 0x0A0F1A);
        int techBorder = (lineAlpha << 24) | (lightMode ? 0x0088BB : 0x00D4BE);
        int techAccent = (lineAlpha << 24) | (lightMode ? 0x0099CC : 0x00FFE0);
        int techText = (alpha << 24) | (lightMode ? 0x1A2A3A : 0xE0F7FF);
        ...
        context.drawText(client.textRenderer, Text.literal(display), -textWidth / 2, 0, techText, !lightMode);
