...
public class RegionPlannerListScreen extends AbstractPhoneScreen {
    ...
    protected Text getCustomHelpTooltip(int mouseX, int mouseY) {
        ...
        int offSwitchY = contentY + s(21);
        if (mouseX >= offSwitchX && mouseX <= offSwitchX + offSwitchW && mouseY >= offSwitchY && mouseY <= offSwitchY + offSwitchH) {
            return Text.translatable("phone.tzz_mod.help.region_off_hand");
        }
        ...
        for (int index = 0; index < rows.size(); index++) {
            ...
            int switchX = contentX + contentWidth - s(32);
            int switchY = drawY + (rowHeight - switchH) / 2;
            if (!row.draft() && mouseX >= switchX && mouseX <= switchX + switchW && mouseY >= switchY && mouseY <= switchY + switchH) {
                return Text.translatable("phone.tzz_mod.help.region_row_visibility");
            }
            return row.draft()
                    ? Text.translatable("phone.tzz_mod.help.region_draft_row")
                    : Text.translatable("phone.tzz_mod.help.region_row_open");
        }
        return Text.empty();
    }
