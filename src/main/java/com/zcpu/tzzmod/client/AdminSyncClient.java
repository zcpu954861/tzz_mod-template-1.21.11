...
public final class AdminSyncClient {
    ...
    public static void register() {
        ClientPlayNetworking.registerGlobalReceiver(AdminModePayload.ID, (payload, context) ->
                context.client().execute(() -> ForcedHudClient.setServerEnforcedHud(payload.recording()))
        );

        ClientPlayConnectionEvents.DISCONNECT.register((handler, client) -> ForcedHudClient.setServerEnforcedHud(null));
    }
}
