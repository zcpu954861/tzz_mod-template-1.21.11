...
.append("""
                ...
                function timerActionRow(bucket,action,index){const prefix=`timer-${bucket}-${index}`, type=String(action.type||'message').toLowerCase(), d=appState.timerEdit||{}, owner=`data-timer-${bucket.replace(/[^a-zA-Z0-9_-]/g,'-')}-action-condition-gate-picker`, gatePicker=actionConditionGatePicker(d,action,index,timerActionBucketTargetType(bucket),action.actionConditionGateTargetId||`timer:${d.id||'draft'}:${bucket}:action:${index}`,owner,'syncTimerDraft()');return `<article class="wa-action-row"><header><div><strong>#${index+1} ${esc(labelActionType(type))}</strong><small>${esc(action.summary||action.timerActionSummary||action.stateActionSummary||action.value||'尚未配置')}</small></div><select id="${prefix}-type" class="select" onchange='changeTimerActionType(${jsString(bucket)},${index})'>${actionTypeOptions(type)}</select></header><div class="wa-action-editor-grid">${timerActionValueEditor(prefix,action)}${gatePicker}<label class="switch-row">启用<input id="${prefix}-enabled" type="checkbox" ${action.enabled!==false?'checked':''} onchange="syncTimerDraft()"></label><label>冷却 tick<input id="${prefix}-cooldown" class="input" type="number" min="0" step="1" value="${esc(action.cooldownTicks||0)}" oninput="syncTimerDraft()"></label></div><div class="inline-actions"><button class="wa-btn ghost" type="button" ${index===0?'disabled':''} onclick='moveTimerAction(${jsString(bucket)},${index},-1)'>上移</button><button class="wa-btn ghost" type="button" ${index>=(appState.timerEdit?.[bucket]||[]).length-1?'disabled':''} onclick='moveTimerAction(${jsString(bucket)},${index},1)'>下移</button><button class="wa-btn danger" type="button" onclick='removeTimerAction(${jsString(bucket)},${index})'>删除</button></div></article>`;}
                function timerActionValueEditor(prefix,action){const type=String(action.type||'message').toLowerCase();if(type==='state_variable')return stateActionEditor(prefix,action,'','syncTimerDraft()','syncTimerDraft();rerenderTimerEditor()','data-timer-state-action-fields="true"');if(type==='timer_start'||type==='timer_cancel')return timerActionEditor(prefix,action,'','syncTimerDraft()','syncTimerDraft();rerenderTimerEditor()','data-timer-nested-timer-action-fields="true"');if(type==='signal')return `<label>Signal 频道<input id="${prefix}-value" class="input" maxlength="128" value="${esc(action.value||'')}" oninput="syncTimerDraft()" placeholder="timer.done"></label>`;if(type==='command')return `<label>命令<input id="${prefix}-value" class="input" maxlength="512" value="${esc(action.value||'')}" oninput="syncTimerDraft()" placeholder="say timer done"></label>`;if(type==='sound')return `<label>音效 ID<input id="${prefix}-value" class="input" maxlength="128" value="${esc(action.value||'minecraft:entity.experience_orb.pickup')}" oninput="syncTimerDraft()"></label>`;return `<label>消息<textarea id="${prefix}-value" class="input wa-action-textarea" maxlength="500" oninput="syncTimerDraft()">${esc(action.value||'')}</textarea></label>`;}
