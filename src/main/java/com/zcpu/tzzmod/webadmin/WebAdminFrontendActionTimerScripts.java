...
.append("""
                ...
                function timerActionSummaryList(actions){return (actions||[]).length?`<div class="list-stack">${actions.map((a,i)=>`<div class="event-row"><strong>#${i+1} ${esc(labelActionType(a.type))}</strong><span>${esc(typedActionDisplaySummary(a))}</span><span class="muted">${a.enabled===false?'禁用':'启用'} · cooldown ${esc(a.cooldownTicks||0)}</span></div>`).join('')}</div>`:empty('尚未配置 action。');}
...
.append("""
                ...
                function signalListenerActionListRow(detail,action,index,draft){const pending=draft.pendingDeleteIndex===index?signalListenerActionDeleteConfirm(detail,action,index):'', locked=lockHeldByOther(detail.lockStatus), gateText=action.conditionGroupId?`已配置：${action.conditionGroupId}`:'未配置', summary=typedActionDisplaySummary(action);return `<article class="wa-action-row signal-listener-action-row"><header><div><strong>#${index+1} ${esc(labelActionType(action.type))}</strong><small>${esc(summary)}</small></div><span class="pill ${action.enabled?'ok':'warning'}">${action.enabled?'启用':'禁用'}</span></header><div class="identity-grid"><span class="k">类型</span><span class="v">${esc(labelActionType(action.type))} <span class="muted">${esc(action.type||'')}</span></span><span class="k">摘要</span><span class="v">${esc(summary)}</span><span class="k">单条条件</span><span class="v" data-action-condition-gate-summary="true">${esc(gateText)}</span><span class="k">冷却</span><span class="v">${esc(action.cooldownTicks||0)} ticks</span></div>${pending}<div class="inline-actions">${canEditSignalListenerActions()?`${waButton('编辑','settings',locked?'disabled':htmlHandler(`openSignalListenerActionEditModal(${jsString(detail.listenerId)},${index},true)`),'ghost')} ${waButton('删除','critical-issue',locked?'disabled':htmlHandler(`requestDeleteSignalListenerAction(${jsString(detail.listenerId)},${index})`),'danger')}`:''}</div></article>`;}
                """)
.append("""
                function signalListenerActionDeleteConfirm(detail,action,index){return `<div class="readonly-note danger" data-signal-listener-action-delete-confirm="true" data-danger-confirm-modal="true"><strong>确认删除 #${index+1} ${esc(labelActionType(action.type))}</strong><span>摘要：${esc(typedActionDisplaySummary(action))}。不需要输入 ID 或名称。</span><div class="inline-actions"><button class="wa-btn danger" type="button" onclick='confirmDeleteSignalListenerAction(${jsString(detail.listenerId)},${index})'>确认删除</button><button class="wa-btn ghost" type="button" onclick='cancelDeleteSignalListenerAction()'>取消</button></div></div>`;}
