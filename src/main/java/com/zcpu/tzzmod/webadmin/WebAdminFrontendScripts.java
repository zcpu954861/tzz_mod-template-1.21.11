...
public final class WebAdminFrontendScripts {
    ...
    public static String appJs() {
        return new StringBuilder()
                ...
                async function loadRuntimeConditionGateOptions(targetTypes,targetId){const targets=[...new Set((targetTypes||[]).filter(Boolean))];const pairs=await Promise.all(targets.map(async targetType=>{try{return [targetType,await api(`/api/webadmin/condition-groups/available?targetType=${encodeURIComponent(targetType)}&targetId=${encodeURIComponent(targetId||'')}`)];}catch(err){return [targetType,{groups:[],error:err.message||'条件组候选加载失败'}];}}));return Object.fromEntries(pairs);}
                ...
                async function prepareActionRelayActionsDraft(deviceId,acquireLock=false){
                  ...
                  const conditionGateOptions=await loadRuntimeConditionGateOptions(['ACTION_RELAY'],canonicalId);
                ...
                function signalListenerActionSummaryCard(actionInfo){
                  ...
                  return `<div class="signal-listener-action-summary" data-signal-listener-action-summary-card="true" data-signal-listener-action-preview-hidden="true"><div class="wa-detail-stat-grid">${detailStatGrid([{label:'动作数量',value:actions.length,sub:'虚拟监听器动作',icon:'action-binding',kind:actions.length?'ok':'warning'},{label:'类型摘要',value:typeSummary,sub:'在动作列表中查看明细',icon:'action-total'}])}</div><p class="muted">动作明细不在详情页展开；点击“${manageLabel}”查看当前列表，写操作会受编辑锁保护。</p><div class="inline-actions">${waButton(manageLabel,'action-relay',manageAttrs,locked?'ghost is-locked':'primary')}${canEdit?waButton('添加动作','plus',addAttrs,locked?'ghost is-locked':'ghost'):''}${lockBadge}</div></div>`;
                }
                ...
                async function openSignalListenerActionAddModal(listenerId,returnToList=false){
                  ...
                  try{
                    ...
                    const [detail,channelOptions]=await Promise.all([api(`/api/webadmin/signal-listeners/${encodeURIComponent(listenerId)}/actions`),loadSignalChannelOptions()]);
                    const result=await acquireSignalListenerActionsLock(detail.listenerId||listenerId);
                    if(!result.success){openReadonlySignalListenerActionListFromLock(detail,listenerId,result,'无法获取编辑锁，动作列表将以只读状态打开。');return;}
                    const lock=result.data?.lock||{};
                    appState.signalListenerActionsEdit={mode:'addAction',detail,draft:{listenerId:detail.listenerId||listenerId,returnToList,returnListUiState,type:'signal',value:'',enabled:true,requiresOp:false,cooldownTicks:0,notifyOps:false,expectedFingerprint:detail.expectedFingerprint||'',lockId:lock.lockId||'',lock,errors:[],saving:false,channelOptions,channelOptionsError:appState.channelOptionsError,channelComboOpen:false,channelComboIndex:0,channelComboQuery:'',channelComboSearchActive:false}};
                ...
                function signalListenerActionAddForm(d){const errs=(d.errors||[]).length?`<ul class="validation-list">${d.errors.map(e=>`<li>${esc(e.message||'输入未通过校验')}</li>`).join('')}</ul>`:'';const commandOnly=String(d.type||'').toLowerCase()==='command'?`<label class="switch-row"><span>需要 OP 权限</span><input id="signal-listener-action-requires-op" type="checkbox" ${d.requiresOp?'checked':''} onchange="syncSignalListenerActionDraft()"></label><label class="switch-row"><span>通知 OP</span><input id="signal-listener-action-notify-ops" type="checkbox" ${d.notifyOps?'checked':''} onchange="syncSignalListenerActionDraft()"></label>`:'';const lockLine=d.lockId?`<div class="readonly-note" data-signal-listener-action-current-lock="true">正在编辑虚拟监听器动作，编辑锁到期：${esc(formatDateTime(d.lock?.expiresAt))}</div>`:'';const editing=Number.isInteger(Number(d.actionIndex));return `<form class="edit-form" data-signal-listener-action-dynamic-fields="true" data-signal-listener-single-action-edit="true" data-signal-listener-action-edit-uses-dynamic-fields="true" data-signal-listener-action-edit-preserves-order="true" data-signal-listener-action-preserve-scroll="true" data-action-modal-validation-preserves-scroll="true" onsubmit="event.preventDefault();${editing?'saveSignalListenerActionEdit()':'saveSignalListenerActionAdd()'}">${lockLine}<label>动作类型<select id="signal-listener-action-type" class="select" onchange="changeSignalListenerActionType()">${signalListenerActionTypeOptions(d.type)}</select></label>${signalListenerActionValueEditor(d)}<label>启用<select id="signal-listener-action-enabled" class="select" onchange="syncSignalListenerActionDraft()"><option value="true" ${d.enabled?'selected':''}>启用</option><option value="false" ${!d.enabled?'selected':''}>禁用</option></select></label><label>冷却时间（ticks）<input id="signal-listener-action-cooldown" class="input" type="number" min="0" step="1" value="${esc(d.cooldownTicks||0)}" oninput="syncSignalListenerActionDraft()"></label>${commandOnly}<p class="readonly-note" data-action-update-keeps-listener-base-fields="true">不会提供 raw JSON；只保存当前动作类型需要的字段，不改变监听器 enabled/channel/cooldown/name，也不改变 SignalBridge 运行时语义。</p>${errs}</form>`;}
                function syncSignalListenerActionDraft(openMenu=false){const d=appState.signalListenerActionsEdit?.draft;if(!d)return;d.type=document.getElementById('signal-listener-action-type')?.value||d.type||'signal';d.value=document.getElementById('signal-listener-action-value')?.value ?? d.value ?? '';d.enabled=(document.getElementById('signal-listener-action-enabled')?.value ?? String(d.enabled))==='true';d.cooldownTicks=Number(document.getElementById('signal-listener-action-cooldown')?.value ?? d.cooldownTicks ?? 0);d.requiresOp=!!document.getElementById('signal-listener-action-requires-op')?.checked;d.notifyOps=!!document.getElementById('signal-listener-action-notify-ops')?.checked;if(openMenu){d.channelComboOpen=true;d.channelComboIndex=0;setChannelComboQuery(d,d.value);}}
                ...
                function modalSnapshot(kind,draft){
                  ...
                  if(k==='signal_listener_action')return JSON.stringify({type:String(draft.type||'signal'),value:String(draft.value||''),enabled:draft.enabled!==false,requiresOp:!!draft.requiresOp,cooldownTicks:Number(draft.cooldownTicks||0),notifyOps:!!draft.notifyOps});
                  ...
                  if(k==='region_controller_action')return JSON.stringify({trigger:String(draft.trigger||''),type:String(draft.type||'signal'),value:String(draft.value||''),enabled:draft.enabled!==false,requiresOp:!!draft.requiresOp,cooldownTicks:Number(draft.cooldownTicks||0),notifyOps:!!draft.notifyOps});
                ...
                function renderRegionControllerList(focusId,options={}){
                  ...
                  const rendered=setView(`<section class="wa-page">
                    ${waPageHead('区域控制器','创建、编辑和审计 RegionController enter / exit / stay 触发配置；不实现 ConditionEngine、路径可视化或 raw JSON。',`${canEditRegionController()?waButton('创建区域控制器','plus',htmlHandler('openRegionControllerCreateModal()'),'primary'):waButton('创建区域控制器','plus','disabled','ghost')}${waButton('导入配置','upload','disabled','ghost')}${waButton('导出配置','download','disabled','ghost')}`)}
                    <section class="wa-card-grid wa-metrics-5">
                ...
                async function renderRegionControllerDetail(id,options={}){
                  ...
                  setView(`<section class="wa-page wa-detail-shell" data-detail-kind="region-controller">
                    ...
                    ${detailFixedLayout([
                      ...
                      detailCard('安全边界',`<div class="list-stack"><div class="event-row"><strong>无 raw JSON</strong><span>只编辑已有字段和 ActionConfig entry。</span></div><div class="event-row"><strong>无 ConditionEngine</strong><span>不新增条件系统、路径图或虚拟监听器。</span></div><div class="event-row"><strong>写入安全</strong><span>权限、CSRF、edit lock、expectedFingerprint、audit、realtime。</span></div></div>`)
                    ],[
                ...
                function regionControllerActionListRow(detail,trigger,action,index,draft){const pending=draft.pendingDeleteIndex===index?regionControllerActionDeleteConfirm(detail,trigger,action,index):'';return `<article class="wa-action-row region-controller-action-row"><header><div><strong>#${index+1} ${esc(labelActionType(action.type))}</strong><small>${esc(cleanActionSummary(action.summary||action.value||'尚未配置'))}</small></div><span class="pill ${action.enabled?'ok':'warning'}">${action.enabled?'启用':'禁用'}</span></header><div class="identity-grid"><span class="k">类型</span><span class="v">${esc(labelActionType(action.type))} <span class="muted">${esc(action.type||'')}</span></span><span class="k">摘要</span><span class="v">${esc(cleanActionSummary(action.summary||action.value||''))}</span><span class="k">冷却</span><span class="v">${esc(action.cooldownTicks||0)} ticks</span></div>${pending}<div class="inline-actions">${canEditRegionController()?waButton('删除','critical-issue',lockHeldByOther(detail.lockStatus)?'disabled':htmlHandler(`requestDeleteRegionControllerAction(${jsString(detail.id)},${jsString(trigger)},${index})`),'danger'):''}</div></article>`;}
                ...
                async function openRegionControllerActionAddModal(controllerId,trigger,returnToList=false){if(!canEditRegionController())return;try{const [detail,channelOptions]=await Promise.all([api(`/api/webadmin/region-controllers/${encodeURIComponent(controllerId)}`),loadSignalChannelOptions()]);const result=await acquireRegionControllerLock(detail.id);if(!result.success){toast(result.message||'无法获取编辑锁');return;}const lock=result.data?.lock||{};appState.regionControllerEdit={mode:'addAction',detail,draft:{id:detail.id,trigger,returnToList,type:'signal',value:'',enabled:true,requiresOp:false,cooldownTicks:0,notifyOps:false,expectedFingerprint:detail.expectedFingerprint||'',lockId:lock.lockId||'',lock,errors:[],saving:false,channelOptions,channelOptionsError:appState.channelOptionsError,channelComboOpen:false,channelComboIndex:0,channelComboQuery:'',channelComboSearchActive:false}};scheduleRegionControllerLockHeartbeat();showRegionControllerActionAddModal();}catch(err){toast(err.message||'无法添加动作');}}
                ...
                function showRegionControllerActionAddModal(uiState=null){const d=appState.regionControllerEdit?.draft;if(!d)return;markModalInitialSnapshot('region_controller_action',d);openWebAdminModal(`添加${regionControllerTriggerLabel(d.trigger)}`,regionControllerActionAddForm(d),editModalFooter(d.saving),{className:'wa-config-modal',onClose:async()=>{await cancelRegionControllerEdit(d.id,true);await dismissWebAdminModal();},syncBeforeClose:()=>syncRegionControllerActionDraft(),dirtyCheck:()=>modalDraftDirty('region_controller_action',appState.regionControllerEdit?.draft)});restoreRegionControllerModalUiState(uiState);}
                ...
                function regionControllerActionAddForm(d){const errs=(d.errors||[]).length?`<ul class="validation-list">${d.errors.map(e=>`<li>${esc(e.message||'输入未通过校验')}</li>`).join('')}</ul>`:'';const commandOnly=String(d.type||'').toLowerCase()==='command'?`<label class="switch-row"><span>需要 OP 权限</span><input id="region-controller-action-requires-op" type="checkbox" ${d.requiresOp?'checked':''} onchange="syncRegionControllerActionDraft()"></label><label class="switch-row"><span>通知 OP</span><input id="region-controller-action-notify-ops" type="checkbox" ${d.notifyOps?'checked':''} onchange="syncRegionControllerActionDraft()"></label>`:'';return `<form class="edit-form" data-region-action-dynamic-fields="true" data-region-action-preserve-scroll="true" onsubmit="event.preventDefault();saveRegionControllerActionAdd()"><label>动作类型<select id="region-controller-action-type" class="select" onchange="changeRegionControllerActionType()">${regionControllerActionTypeOptions(d.type)}</select></label>${regionControllerActionValueEditor(d)}<label>启用<select id="region-controller-action-enabled" class="select" onchange="syncRegionControllerActionDraft()"><option value="true" ${d.enabled?'selected':''}>启用</option><option value="false" ${!d.enabled?'selected':''}>禁用</option></select></label><label>冷却时间（ticks）<input id="region-controller-action-cooldown" class="input" type="number" min="0" step="1" value="${esc(d.cooldownTicks||0)}" oninput="syncRegionControllerActionDraft()"></label>${commandOnly}<p class="readonly-note">不会提供 raw JSON；只保存当前动作类型需要的字段。</p>${errs}</form>`;}
                function syncRegionControllerActionDraft(){const d=appState.regionControllerEdit?.draft;if(!d)return;d.type=document.getElementById('region-controller-action-type')?.value||d.type||'signal';d.value=document.getElementById('region-controller-action-value')?.value ?? d.value ?? '';d.enabled=(document.getElementById('region-controller-action-enabled')?.value ?? String(d.enabled))==='true';d.cooldownTicks=Number(document.getElementById('region-controller-action-cooldown')?.value ?? d.cooldownTicks ?? 0);d.requiresOp=!!document.getElementById('region-controller-action-requires-op')?.checked;d.notifyOps=!!document.getElementById('region-controller-action-notify-ops')?.checked;}
