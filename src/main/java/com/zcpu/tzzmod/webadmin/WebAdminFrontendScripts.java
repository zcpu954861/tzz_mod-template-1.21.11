...
public final class WebAdminFrontendScripts {
    ...
    public static String appJs() {
        return new StringBuilder()
                ...
                const TZZ_WEBADMIN_ASSET_VERSION='8.5-condition-editor-p0-3';
                ...
                async function renderConditionGroupDetail(raw,options={}){
                  ...
                  try{
                    ...
                    ${detailHeader({back:backButton(route,'返回条件组列表'),iconName:detail.iconKey||'doctor-overview',kicker:'条件组 / 8.5 WebAdmin Condition Editor',title:detail.displayName||detail.id,subtitle:'只保存 WebAdmin 条件组配置，不挂载任何运行时。',copyValue:detail.id,badges:[conditionGroupStatusPill(detail.enabled!==false),`<span class="pill info">fingerprint ${esc((detail.fingerprint||'').slice(0,10))}</span>`],actions})}
                      ${lockLine}
                ...
                function showConditionGroupEditModal(uiState=null){
                  ...
                  const body=`<form class="edit-form condition-group-form" data-condition-group-structured-editor="true" data-condition-main-compact-card-model="true" onsubmit="event.preventDefault();saveConditionGroup()">${lockLine}${errors}
                    ...
                    <p class="muted">点击节点或子组卡片进入独立编辑弹窗。结构化编辑器只保存 Condition Group 配置，不提供 raw JSON 编辑，不挂载任何运行时。</p></form>`;
