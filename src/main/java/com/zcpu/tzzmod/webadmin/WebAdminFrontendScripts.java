...
public final class WebAdminFrontendScripts {
    ...
    public static String appJs() {
        return new StringBuilder()
                ...
                const TZZ_WEBADMIN_ASSET_VERSION='8.9-single-action-gates';
                ...
                function captureViewState(){
                  ...
                  return {
                    ...
                    nestedScrolls:[...((view&&view.querySelectorAll)?view.querySelectorAll('[data-condition-type-catalog] .condition-type-list,[data-condition-node-compact-list],[data-condition-preview-panel],.condition-preview-result'):[])].map((el,i)=>({key:el.dataset.scrollKey||el.dataset.conditionScrollKey||el.getAttribute('data-condition-node-compact-list')||el.getAttribute('data-condition-preview-panel')||el.className||`condition-scroll-${i}`,top:el.scrollTop||0,left:el.scrollLeft||0}))
                  };
                }
                ...
                function restoreViewState(state){
                  ...
                  requestAnimationFrame(()=>{
                    ...
                    [...view.querySelectorAll('[data-condition-type-catalog] .condition-type-list,[data-condition-node-compact-list],[data-condition-preview-panel],.condition-preview-result')].forEach((el,i)=>{const key=el.dataset.scrollKey||el.dataset.conditionScrollKey||el.getAttribute('data-condition-node-compact-list')||el.getAttribute('data-condition-preview-panel')||el.className||`condition-scroll-${i}`, saved=(state.nestedScrolls||[]).find(item=>item.key===key);if(saved){el.scrollTop=saved.top||0;el.scrollLeft=saved.left||0;}});
                ...
                function shouldHandleRealtimeEvent(hash,event){
                  ...
                  if(String(hash||'').startsWith('#/logic-chains/'))return ['logic_chain_metadata_changed','signal_emitted','signal_history_appended','history_appended','signal_listener_changed','signal_listener_enabled_changed','signal_listener_action_changed','action_changed','action_config_changed','region_controller_changed','region_event_appended','device_changed','device_config_changed','receiver_changed','virtual_block_device_changed','edit_lock_changed','doctor_issues_changed','doctor_changed','config_changed'].includes(type)||type.startsWith('signal_')||type.startsWith('action_')||type.startsWith('device_')||type.startsWith('region_');
                ...
                async function saveSignalListenerCreateModal(){
                  ...
                  try{
                    ...
                    draft.saving=false;draft.errors=writeResultErrors(result,'创建虚拟监听器失败。');appState.signalListenerCreate=draft;showSignalListenerCreateModal();toast(result.message||'创建虚拟监听器失败。');
                ...
                function signalListenerDeleteDraft(listener,channel=''){
                  return {listener:{...(listener||{}),channel:listener?.channel||channel||''},confirmed:false,reason:'',saving:false,errors:[]};
                }
                ...
                function showSignalListenerDeleteModal(){
                  ...
                  openWebAdminModal('删除虚拟监听器',listenerDeleteModalBody(draft),dangerousModalFooter(draft.saving,'确认删除'),{className:'wa-config-modal',onClose:()=>{appState.signalListenerDelete=null;return dismissWebAdminModal();}});
                }
                ...
                async function deleteSignalListenerFromModal(){
                  ...
                  try{
                    const result=await api(`/api/webadmin/signal-listeners/${encodeURIComponent(listenerId)}/delete`,{method:'POST',headers:{'X-TZZ-WebAdmin-CSRF':csrfToken()},body:JSON.stringify({listenerId,confirmed:draft.confirmed,reason:draft.reason})});
                    if(result.success){
                ...
                function showContainerTemplateSessionModal(deviceId){
                  ...
                  openWebAdminModal('容器内容变化模板',containerTemplateSessionBody(draft),footer,{className:'wa-config-modal wa-container-template-modal',onClose:async()=>closeContainerTemplateSessionModal(draft.deviceId)});
                }
                ...
                async function startContainerTemplateSession(deviceId){
                  ...
                  try{
                    ...
                    const result=await api(`/api/webadmin/virtual-block-devices/${encodeURIComponent(draft.deviceId)}/container-template-session/start`,{method:'POST',headers:{'X-TZZ-WebAdmin-CSRF':csrfToken()},body:JSON.stringify({deviceId:draft.deviceId,targetPlayerName:draft.targetPlayerName,targetPlayerUuid:draft.targetPlayerUuid,lockId:draft.lockId,expectedFingerprint:draft.expectedFingerprint})});
                ...
                function showSingleItemSubmitSessionModal(deviceId){
                  ...
                  const startDisabled=draft.saving||lockHeldByOther(overview.lockStatus)||overview.interactionEnabled===false;
                  ...
                  openWebAdminModal('itemSubmit 条件编辑器',singleItemSubmitSessionBody(draft),footer,{className:'wa-config-modal wa-single-item-submit-modal',onClose:async()=>closeSingleItemSubmitSessionModal(draft.deviceId)});
                }
                function singleItemSubmitSessionBody(draft){
                  ...
                  if(overview.interactionEnabled===false){
                ...
                async function refreshSingleItemSubmitSessionStatus(source='status'){
                  ...
                  draft.status=status.status||draft.status;draft.active=!!status.active;draft.message=status.message||draft.message;draft.sessionId=sessionId||draft.sessionId;
                  if(!draft.active){draft.lockId='';draft.lock=null;stopSingleItemSubmitSessionHeartbeat();stopSingleItemSubmitSessionStatusPoll();cancelSingleItemSubmitCancelConfirm();if(['saved','completed'].includes(String(draft.status||''))){markRealtimeRouteKeyDirty('devices',{type:'config_changed'});if(draft.deviceId){deviceDetailRouteKeys(draft.deviceId,'virtual_block_device').forEach(key=>markRealtimeRouteKeyDirty(key,{type:'config_changed'}));}await refreshSingleItemSubmitSessionOverview(draft.deviceId,true);return;}showSingleItemSubmitSessionModal(draft.deviceId);}
                ...
                function normalizeActionRelayEditableAction(action={}){return {type:String(action.type||'command').toLowerCase(),value:String(action.value||''),enabled:action.enabled!==false,requiresOp:!!action.requiresOp,cooldownTicks:Number(action.cooldownTicks||0),notifyOps:!!action.notifyOps,conditionGroupId:String(action.conditionGroupId||'')};}
                ...
                function syncActionRelayActionsDraftFromForm(deviceId,indexToOpen=null,openMenu=false){
                  ...
                  draft.actions=(draft.actions||[]).map((action,index)=>{const type=String(document.getElementById(`ara-${index}-type`)?.value||action.type||'command').toLowerCase();return {...action,type,value:document.getElementById(`ara-${index}-value`)?.value||'',enabled:!!document.getElementById(`ara-${index}-enabled`)?.checked,requiresOp:!!document.getElementById(`ara-${index}-requires-op`)?.checked,cooldownTicks:Number(document.getElementById(`ara-${index}-cooldown`)?.value||0),notifyOps:!!document.getElementById(`ara-${index}-notify`)?.checked,conditionGroupId:document.getElementById(`action-condition-data-action-relay-action-condition-gate-picker-${index}`)?.value||''};});
                ...
                function signalListenerActionTypeOptions(value){return ['signal','command','message','sound'].map(type=>`<option value="${type}" ${String(value||'signal').toLowerCase()===type?'selected':''}>${esc(labelActionType(type))} · ${type}</option>`).join('');}
                function signalListenerActionValueEditor(d){const type=String(d.type||'signal').toLowerCase();if(type==='signal')return `<label class="wa-action-value-field" data-signal-listener-action-signal-only="true">信号频道${renderSignalListenerActionChannelCombo(d)}<span id="signal-listener-action-channel-hint" class="readonly-note">${channelHintHtml(d.value,d.channelOptions||appState.channelOptions||[],d.channelOptionsError||appState.channelOptionsError)}</span></label>`;if(type==='command')return `<label class="wa-action-value-field" data-signal-listener-action-command-fields="true">命令<input id="signal-listener-action-value" class="input" maxlength="512" value="${esc(d.value||'')}" placeholder="say hello" oninput="syncSignalListenerActionDraft()"><span class="readonly-note">不要输入开头的 /；会阻断 stop/op/ban/kick/whitelist/save-off/save-on/reload 以及 execute ... run 的嵌套危险服务器管理命令。</span></label>`;if(type==='message')return `<label class="wa-action-value-field" data-signal-listener-action-message-field="true">消息内容<textarea id="signal-listener-action-value" class="input wa-action-textarea" maxlength="500" oninput="syncSignalListenerActionDraft()">${esc(d.value||'')}</textarea></label>`;if(type==='sound')return `<label class="wa-action-value-field" data-signal-listener-action-sound-field="true">音效 ID<input id="signal-listener-action-value" class="input" maxlength="128" value="${esc(d.value||'minecraft:entity.experience_orb.pickup')}" placeholder="minecraft:entity.experience_orb.pickup" oninput="syncSignalListenerActionDraft()"><span class="muted">当前底层保存音效 ID；音量/音调按现有 ActionConfig 能力。</span></label>`;return `<div class="readonly-note danger">暂不支持该动作类型：${esc(type)}</div>`;}
                ...
                function syncSignalListenerActionDraft(openMenu=false){const d=appState.signalListenerActionsEdit?.draft;if(!d)return;d.type=document.getElementById('signal-listener-action-type')?.value||d.type||'signal';d.value=document.getElementById('signal-listener-action-value')?.value ?? d.value ?? '';d.conditionGroupId=document.getElementById(`action-condition-data-signal-listener-action-condition-gate-picker-${Number(d.actionIndex||0)}`)?.value||'';d.enabled=(document.getElementById('signal-listener-action-enabled')?.value ?? String(d.enabled))==='true';d.cooldownTicks=Number(document.getElementById('signal-listener-action-cooldown')?.value ?? d.cooldownTicks ?? 0);d.requiresOp=!!document.getElementById('signal-listener-action-requires-op')?.checked;d.notifyOps=!!document.getElementById('signal-listener-action-notify-ops')?.checked;if(openMenu){d.channelComboOpen=true;d.channelComboIndex=0;setChannelComboQuery(d,d.value);}}
                function changeSignalListenerActionType(){syncSignalListenerActionDraft();const d=appState.signalListenerActionsEdit?.draft;if(!d)return;const next=String(document.getElementById('signal-listener-action-type')?.value||'signal').toLowerCase();d.type=next;d.value=next==='sound'?'minecraft:entity.experience_orb.pickup':'';d.requiresOp=false;d.notifyOps=false;showSignalListenerActionAddModal(captureSignalListenerActionModalUiState());}
                function renderSignalListenerActionChannelCombo(d){const open=d.channelComboOpen?' open':'';return `<div id="signal-listener-action-channel-combo" class="channel-combo signal-listener-action-channel-combo${open}"><div class="channel-combo-control"><input id="signal-listener-action-value" class="input" maxlength="128" value="${esc(d.value||'')}" placeholder="选择已有频道或输入新频道" autocomplete="off" role="combobox" aria-expanded="${d.channelComboOpen?'true':'false'}" aria-controls="signal-listener-action-channel-menu" onfocus='openSignalListenerActionChannelMenu()' oninput='syncSignalListenerActionDraft(true)' onkeydown='handleSignalListenerActionChannelKey(event)'><button class="channel-combo-toggle" type="button" onclick='toggleSignalListenerActionChannelMenu()' aria-label="显示已有频道">⌄</button></div><div id="signal-listener-action-channel-menu" class="channel-combo-menu" role="listbox">${signalListenerActionChannelOptionsHtml(d)}</div></div>`;}
                ...
                function modalSnapshot(kind,draft){
                  ...
                  if(k==='signal_listener_action')return JSON.stringify({type:String(draft.type||'signal'),value:String(draft.value||''),enabled:draft.enabled!==false,requiresOp:!!draft.requiresOp,cooldownTicks:Number(draft.cooldownTicks||0),notifyOps:!!draft.notifyOps,conditionGroupId:String(draft.conditionGroupId||'')});
                  ...
                  if(k==='region_controller_action')return JSON.stringify({trigger:String(draft.trigger||''),type:String(draft.type||'signal'),value:String(draft.value||''),enabled:draft.enabled!==false,requiresOp:!!draft.requiresOp,cooldownTicks:Number(draft.cooldownTicks||0),notifyOps:!!draft.notifyOps,conditionGroupId:String(draft.conditionGroupId||'')});
                  ...
                  setView(`<section class="wa-page wa-detail-shell" data-detail-kind="region-controller">
                    ...
                    <section class="wa-detail-first-row">
                      ...
                      ${detailCard('动作统计',detailStatGrid([
                        ...
                        {label:'信号频道',value:(detail.boundChannels||[]).length,sub:'signal 动作',icon:'active-channel'}
                      ]))}
                    </section>
                    ${detailFixedLayout([
                      ...
                      detailCard('Doctor / Runtime',`<div class="list-stack"><div class="event-row"><strong>Doctor</strong>${pill(detail.doctorStatus||'UNKNOWN')}<span class="muted">当前 RegionController 运行时不新增新语义。</span></div><div class="event-row"><strong>绑定频道</strong>${(detail.boundChannels||[]).length?(detail.boundChannels||[]).map(c=>channelButton(c)).join(''):'<span class="muted">无 signal action 频道</span>'}</div></div>`,'','detail-card-stretchable'),
                ...
                function syncRegionControllerActionDraft(){const d=appState.regionControllerEdit?.draft;if(!d)return;d.type=document.getElementById('region-controller-action-type')?.value||d.type||'signal';d.value=document.getElementById('region-controller-action-value')?.value ?? d.value ?? '';d.conditionGroupId=document.getElementById(`action-condition-data-region-controller-action-condition-gate-picker-${Number(d.actionIndex||0)}`)?.value||'';d.enabled=(document.getElementById('region-controller-action-enabled')?.value ?? String(d.enabled))==='true';d.cooldownTicks=Number(document.getElementById('region-controller-action-cooldown')?.value ?? d.cooldownTicks ?? 0);d.requiresOp=!!document.getElementById('region-controller-action-requires-op')?.checked;d.notifyOps=!!document.getElementById('region-controller-action-notify-ops')?.checked;}
                function changeRegionControllerActionType(){syncRegionControllerActionDraft();const d=appState.regionControllerEdit?.draft;if(!d)return;const next=String(document.getElementById('region-controller-action-type')?.value||'signal').toLowerCase();d.type=next;d.value=next==='sound'?'minecraft:entity.experience_orb.pickup':'';d.requiresOp=false;d.notifyOps=false;showRegionControllerActionAddModal(captureRegionControllerModalUiState());}
                function renderRegionControllerActionChannelCombo(d){const open=d.channelComboOpen?' open':'';return `<div id="region-controller-action-channel-combo" class="channel-combo region-controller-action-channel-combo${open}"><div class="channel-combo-control"><input id="region-controller-action-value" class="input" maxlength="128" value="${esc(d.value||'')}" placeholder="选择已有频道或输入新频道" autocomplete="off" role="combobox" aria-expanded="${d.channelComboOpen?'true':'false'}" aria-controls="region-controller-action-channel-menu" onfocus='openRegionControllerActionChannelMenu()' oninput='syncRegionControllerActionDraft(true)' onkeydown='handleRegionControllerActionChannelKey(event)'><button class="channel-combo-toggle" type="button" onclick='toggleRegionControllerActionChannelMenu()' aria-label="显示已有频道">⌄</button></div><div id="region-controller-action-channel-menu" class="channel-combo-menu" role="listbox">${regionControllerActionChannelOptionsHtml(d)}</div></div>`;}
                ...
                """).append("""
                function logicChainRootTypeLabel(type){return {channel:'频道',device:'设备',listener:'虚拟监听器',receiver:'接收器',relay:'动作继电器',region:'区域',region_controller:'区域控制器',action:'动作'}[String(type||'').toLowerCase()]||String(type||'root');}
                function logicChainNodeTypeLabel(type,refType){const t=String(type||'').toLowerCase(), r=String(refType||'').toLowerCase();if(t==='producer')return '触发源';if(t==='channel')return '频道';if(t==='consumer')return r==='listener'?'虚拟监听器':(r==='receiver'?'信号接收器':(r==='action_relay'?'动作继电器':'消费者'));if(t==='action')return '动作';if(t==='downstream_channel')return '下游频道';if(t==='root')return 'root';return t||'节点';}
                function logicChainNodeIcon(node){const type=String(node?.type||'').toLowerCase(), ref=String(node?.refType||'').toLowerCase();if(type==='channel'||type==='downstream_channel')return 'active-channel';if(type==='producer')return ref==='region_controller'?'region-controller':'signal-device';if(type==='consumer')return ref==='listener'?'consumer-listener':(ref==='receiver'?'consumer-receiver':'consumer-relay');if(type==='action')return 'action-binding';return 'action-overview';}
                ...
                function logicChainFilter(items){const f=appState.logicChainFilters||{}, search=String(f.search||'').toLowerCase();return (items||[]).filter(item=>{const hay=[item.id,item.displayName,item.rootType,item.rootRef,item.rootChannel,item.doctorStatus,item.metadata?.note,item.upstreamSourceLabel,item.parentRootRef,item.hierarchyLevel].join(' ').toLowerCase();if(search&&!hay.includes(search))return false;if(f.status&&f.status!=='ALL'&&String(item.doctorStatus||'UNKNOWN').toUpperCase()!==f.status)return false;if(f.saved==='SAVED'&&!item.saved)return false;if(f.saved==='AUTO'&&item.saved)return false;return true;});}
                """).append("""
                ...
                function conditionGroupListPanel(groups){
                  ...
                  <div class="wa-table-scroll"><table class="wa-table"><thead><tr><th>条件组</th><th>状态</th><th>节点</th><th>最近修改</th><th>操作</th></tr></thead><tbody>
                    ${page.items.map(g=>`<tr class="condition-group-row" data-condition-group-id="${esc(g.id)}" ${navigationAttr(conditionGroupHash(g.id))}><td><div class="wa-row-title"><span class="wa-icon-bubble icon-bubble-doctor-overview">${icon(g.iconKey||'doctor-overview')}</span><span><strong>${esc(g.displayName||g.id)}</strong><small class="device-subtitle">${esc(g.id)} · ${esc(g.note||'暂无备注')}</small></span></div></td><td>${conditionGroupStatusPill(g.enabled!==false)}</td><td>${esc(g.nodeCount||0)}</td><td>${esc(formatDateTime(g.updatedAt||g.createdAt))}</td><td><div class="wa-action-cell">${waButton('查看','doctor-overview',navigationAttr(conditionGroupHash(g.id)),'ghost')}</div></td></tr>`).join('')||`<tr><td colspan="5">${empty('暂无条件组。')}</td></tr>`}
                ...
                function makeConditionDraftFromDetail(detail,mode='edit'){
                  ...
                  return {mode,id:detail?.id||'',displayName:detail?.displayName||'',note:detail?.note||'',iconKey:detail?.iconKey||'doctor-overview',enabled:detail?.enabled!==false,tagsText:(detail?.tags||[]).join(', '),expectedFingerprint:detail?.fingerprint||detail?.expectedFingerprint||'',lockId:'',lock:null,errors:[],saving:false,groupDefinition:def,definitionMissing:mode!=='create'&&!hasDefinition};
                }
                ...
                function conditionNodeCompactCard(node,path,isGroup){const p=path.join('.'), meta=conditionTypeMeta(node.type), selected=conditionActiveNodePath()===p, summary=isGroup?`${conditionGroupModeLabel(node.groupMode)} · ${(node.children||[]).length} 个节点`:conditionConfigSummary(node,meta);const title=p?`节点 ${Number(path[path.length-1])+1}`:'Root 条件组';return `<article class="condition-node-compact-card ${node.enabled===false?'disabled':''} ${selected?'active':''}" data-condition-node-compact-card="true" data-condition-node-card-click-opens-editor="true" data-condition-edit-path="${esc(p)}" role="button" tabindex="0" aria-label="编辑 ${esc(title)}"><div class="condition-node-compact-main"><strong>${esc(title)}</strong><span>${esc(meta.displayName||node.type||'未知条件')}</span><small>type: ${esc(node.type||'')} · id: ${esc(node.id||'')}</small><small>${esc(summary||'无关键配置')}</small></div><div class="condition-node-compact-status"><span class="pill ${node.enabled===false?'warning':'ok'}">${node.enabled===false?'停用':'启用'}</span><span class="pill info" data-condition-node-validation-status="true">${esc(isGroup?'子组':'条件')}</span></div><div class="inline-actions condition-node-compact-actions">${waButton('编辑','settings',conditionNodeQuickAttrs(`openConditionNodeEditor(${jsString(p)})`,'edit'),'ghost')}${waButton(node.enabled===false?'启用':'禁用','doctor',conditionNodeQuickAttrs(`toggleConditionNodeEnabled(${jsString(p)})`,'toggle'),'ghost')}${isGroup?waButton('添加条件','check-pass',conditionNodeQuickAttrs(`addConditionLeaf(${jsString(p)})`,'add-leaf'),'ghost')+waButton('添加分组','doctor-overview',conditionNodeQuickAttrs(`addConditionGroup(${jsString(p)})`,'add-group'),'ghost'):''}${p?waButton('删除','channel-error',conditionNodeQuickAttrs(`deleteConditionNode(${jsString(p)})`,'delete'),'danger'):''}</div></article>`;}
                ...
                function renderLogicChainList(focusId,options={}){
                  ...
                  if(setView(`<section class="wa-page" data-logic-chain-list-page="true">
                    ${waPageHead('逻辑链','跨频道只读逻辑链查看器 MVP。逻辑链由 SignalBridge 现有关系推导，不改变运行时。',waButton('刷新','refresh','onclick="renderLogicChainsPage()"','ghost'))}
                    <section class="wa-card-grid wa-metrics-4">
                      ${waMetric('链路候选',chains.length,'保存视图与自动发现频道','channel-total')}
                      ${waMetric('已保存视图',saved,'WebAdmin-only 视图 metadata','action-template','ok')}
                      ${waMetric('涉及频道',uniqueValues(chains.map(c=>c.rootChannel).filter(Boolean)).length,'root channel 去重','active-channel')}
                ...
                function logicChainTable(items){const rows=logicChainHierarchyRows(items);return `<div class="wa-table-scroll" data-logic-chain-sub-chain-hierarchy="true" data-no-flat-all-chains-list="true"><table class="wa-table" data-logic-chain-list-table="true"><thead><tr><th>逻辑链</th><th>层级</th><th>root channel</th><th>上游来源</th><th>频道数</th><th>消费者</th><th>动作</th><th>状态</th><th>操作</th></tr></thead><tbody>${rows.map(item=>logicChainTableRow(item)).join('')}</tbody></table></div>`;}
                function logicChainHierarchyRows(items){const byParent={}, byId={};(items||[]).forEach(item=>{byId[item.id]=item;const parent=item.parentChainId||'';(byParent[parent]||(byParent[parent]=[])).push(item);});Object.values(byParent).forEach(list=>list.sort((a,b)=>Number(a.hierarchyLevel||0)-Number(b.hierarchyLevel||0)||String(a.rootChannel||'').localeCompare(String(b.rootChannel||''))));const top=(items||[]).filter(item=>item.visibleInTopLevel!==false||!byId[item.parentChainId]).sort((a,b)=>String(a.rootChannel||'').localeCompare(String(b.rootChannel||'')));const out=[], seen=new Set();function visit(item,level,ancestorVisible=true){if(!item||seen.has(item.id)||!ancestorVisible)return;seen.add(item.id);out.push({...item,_displayLevel:level});const expanded=appState.logicChainFilters.expanded?.[item.id]!==false;if(expanded)(byParent[item.id]||[]).forEach(child=>visit(child,level+1,true));}top.forEach(item=>visit(item,Number(item.hierarchyLevel||0),true));(items||[]).forEach(item=>{if(!seen.has(item.id)&&(!item.parentChainId||!byId[item.parentChainId]))out.push({...item,_displayLevel:Number(item.hierarchyLevel||0),isReference:true});});return out;}
                function logicChainTableRow(item){const target=logicChainHash(item.id), title=item.displayName||item.rootChannel||item.id, level=Number(item._displayLevel??item.hierarchyLevel??0), hasChildren=Number(item.childrenCount||0)>0, expanded=appState.logicChainFilters.expanded?.[item.id]!==false, sub=item.isSubChain||level>0, reference=item.isReference||item.hasMultipleParents;const toggle=hasChildren?`<button class="wa-icon-btn logic-chain-row-toggle" type="button" onclick='event.preventDefault();event.stopPropagation();toggleLogicChainRow(${jsString(item.id)});' title="${expanded?'折叠子链':'展开子链'}" data-logic-chain-child-toggle-collapses="true" data-expanded-state-keyed-by-chain-id="true">${expanded?'▾':'▸'}</button>`:'<span class="logic-chain-row-spacer"></span>';return `<tr class="wa-clickable-row logic-chain-row level-${Math.min(6,level)}" data-logic-chain-level="${esc(level)}" data-sub-chain="${sub}" data-third-level-sub-chain="${level>=2}" data-multiple-upstream-reference="${item.hasMultipleParents}" data-cycle-hierarchy-guard="${item.hasCycle}" data-self-cycle-not-child-chain="${item.selfCycle}" data-nested-child-chain-collapse="true" ${navDataAttr(target,`查看逻辑链 ${title}`)}><td><span class="device-name logic-chain-row-title" style="padding-left:${Math.min(72,level*22)}px">${toggle}<span class="device-icon">${icon(item.metadata?.effectiveIconKey||'action-binding')}</span><span><strong>${esc(title)}</strong><span class="device-subtitle">${esc(sub?(level>=2?'三级子链':'子链'):(item.saved?'已保存主链':'主链候选'))}${reference?' · 多上游引用':''}${item.selfCycle?' · 自循环':''}${item.hasCycle&&!item.selfCycle?' · 循环引用':''}</span></span></span></td><td>${esc(level===0?'一级主链':(level===1?'二级子链':`${level+1} 级子链`))}</td><td><strong>${esc(item.rootChannel||'-')}</strong><span class="device-subtitle">${esc(logicChainRootTypeLabel(item.rootType))}: ${esc(item.rootRef||'-')}</span></td><td>${item.upstreamSourceLabel?`<span>${esc(item.upstreamSourceLabel)}</span><span class="device-subtitle">父链：${esc(item.parentRootRef||'-')}</span>`:'<span class="muted">无上游，独立主链</span>'}</td><td>${esc(item.channelCount||0)}</td><td>${esc(item.consumerCount||0)}</td><td>${esc(item.actionCount||0)}</td><td>${pill(item.doctorStatus||'UNKNOWN')}</td><td><div class="wa-action-cell"><button class="wa-btn ghost" ${navDataAttr(target,`查看逻辑链 ${title}`)}>查看</button>${item.rootChannel?`<button class="wa-btn ghost" ${navDataAttr(logicChainResolveHash('channel',item.rootChannel),`临时解析 ${item.rootChannel}`)}>解析</button>`:''}</div></td></tr>`;}
                ...
                async function renderLogicChainResolve(hash,options={}){
                  ...
                  renderLogicChainViewer(graph,{fallback:'#/logic-chains',returnTo:isValidReturnHash(params.returnTo)?params.returnTo:'',temporary:true},options);
                }
                async function renderLogicChainDetail(rawId,options={}){
                  ...
                  let graph;try{graph=await api(`/api/webadmin/logic-chains/${encodeURIComponent(chainId)}`)}catch(err){if(options.silent){toast('逻辑链详情实时刷新失败，已保留当前页面。');return;}setView(`<section class="wa-page"><div class="back-row">${backButton(routeInfo,'返回逻辑链')}</div>${waPageHead('逻辑链不可用','该逻辑链 metadata 不存在或 root 无法解析。',waButton('返回列表','action-binding',navigationAttr('#/logic-chains'),'ghost'))}${errorBlock(err.message)}</section>`);return;}
                ...
                function renderLogicChainViewer(graph,routeInfo={},options={}){
                  ...
                  appState.logicChainCanvas.routeInfo={fallback:routeInfo.fallback||'#/logic-chains',returnTo:isValidReturnHash(routeInfo.returnTo)?routeInfo.returnTo:''};
                  const nodes=logicChainNodeMap(graph), metadata=graph?.metadata||{}, stats=graph?.stats||{}, root=graph?.root||{};
                  ...
                  if(appState.logicChainCanvas.graphKey!==graphKey){appState.logicChainCanvas.graphKey=graphKey;appState.logicChainCanvas.panX=0;appState.logicChainCanvas.panY=0;appState.logicChainCanvas.zoom=1;appState.logicChainCanvas.collapsedChannels={};appState.logicChainCanvas.selectedNodeId='';}
                  if(!appState.logicChainCanvas.selectedNodeId||!nodes[appState.logicChainCanvas.selectedNodeId])appState.logicChainCanvas.selectedNodeId=root.id||Object.keys(nodes)[0]||'';
                  ...
                  const actions=[logicChainMetadataAction(graph),waButton('刷新','refresh','onclick="route()"','ghost'),waButton('自动布局','action-binding','onclick="resetLogicChainLayout()"','ghost')].filter(Boolean);
                  const rendered=setView(`<section class="wa-page logic-chain-page" data-logic-chain-viewer-page="true" data-logic-chain-readonly-graph="true" data-no-condition-engine="true" data-no-runtime-node-creation="true">
                    ${detailHeader({back:backButton(appState.logicChainCanvas.routeInfo,'返回逻辑链'),kicker:'WebAdmin / 跨频道逻辑链',iconName:metadata.effectiveIconKey||'action-binding',title,subtitle:'只读思维导图视图：由现有 SignalBridge 关系推导跨频道逻辑树，不修改运行时。',copyValue:metadata.id||metadata.rootRef||metadata.rootChannel,badges:[`<span class="pill info">根节点: ${esc(logicChainRootTypeLabel(metadata.rootType||root.refType))}</span>`,`<span class="pill">${esc(metadata.rootChannel||root.channel||'未解析频道')}</span>`,pill(logicChainGraphDoctorStatus(graph))],actions})}
                    ${warningHtml}
                    <section class="logic-chain-layout">
                      ...
                      <main class="logic-chain-canvas-shell" data-channel-logic-chain-viewer="true" data-logic-chain-mind-map-tree="true" data-curved-connectors="true" data-no-table-like-fixed-lane-layout="true" data-root-channel-consumer-action-downstream-tree="true" data-channel-separates-consumers="true" data-same-channel-consumers-parallel="true" data-action-order-local-only="true" data-downstream-channel-child-subtree="true" data-no-cross-channel-consumer-mixing="true" data-no-cross-channel-long-line-mixing="true">
                        ...
                        ${logicChainCanvas(graph,nodes)}
                      </main>
                      <aside class="logic-chain-right" data-logic-chain-node-detail="true">${logicChainSelectedNodePanel(graph,nodes)}</aside>
                    </section>
                    <section class="logic-chain-footer">${logicChainStats(stats)}${logicChainVersionNote()}</section>
                  </section>`,options);
                  if(rendered)renderIcons(appView());
                }
                function logicChainGraphDoctorStatus(graph){const nodes=graph?.nodes||[];if(nodes.some(n=>String(n.doctorStatus||n.status||'').toUpperCase()==='ERROR'))return 'ERROR';if((graph?.warnings||[]).length||nodes.some(n=>String(n.doctorStatus||n.status||'').toUpperCase()==='WARNING'))return 'WARNING';return 'OK';}
                function logicChainLegend(graph){const stats=graph?.stats||{};return `<h2>图例 / 筛选</h2><div class="logic-chain-legend-list"><span class="logic-chain-chip producer">${icon('signal-device')}触发源</span><span class="logic-chain-chip channel">${icon('active-channel')}频道</span><span class="logic-chain-chip consumer">${icon('consumer-listener')}消费者</span><span class="logic-chain-chip action">${icon('action-binding')}动作 / 继电器</span><span class="logic-chain-chip downstream">${icon('active-channel')}下游频道</span><span class="logic-chain-chip disabled">停用节点</span><span class="logic-chain-chip warning">断链 / 异常</span></div><p class="muted">思维导图模式从左到右表示激活方向，从上到下表示并列分支。</p><p class="muted">同一频道下消费者是并列消费者，不表示严格先后顺序；只有动作列表内部编号表示动作顺序。</p><p class="muted">下游频道表示由上游动作触发的新频道，并作为子树继续展开。</p><p class="muted">当前版本只读，不改变 SignalBridge runtime。条件判断和分支逻辑留到 8.x ConditionEngine 后接入。</p><div class="identity-grid"><span class="k">最大深度</span><span class="v">${esc(stats.maxDepth||3)}</span><span class="k">视图模式</span><span class="v">思维导图树</span></div>`;}
                function logicChainCanvasToolbar(){const zoom=Math.round(Number(appState.logicChainCanvas.zoom||1)*100);return `<div class="logic-chain-canvas-toolbar"><button class="wa-btn ghost" onclick="setLogicChainZoom(-0.1)">-</button><span>${esc(zoom)}%</span><button class="wa-btn ghost" onclick="setLogicChainZoom(0.1)">+</button><button class="wa-btn ghost" onclick="resetLogicChainLayout()">重置视图</button></div>`;}
                function logicChainCanvas(graph,nodes){const zoom=Math.max(.45,Math.min(1.8,Number(appState.logicChainCanvas.zoom||1))), panX=Number(appState.logicChainCanvas.panX||0), panY=Number(appState.logicChainCanvas.panY||0), tree=logicChainBuildTree(graph,nodes), layout=tree?logicChainLayoutTree(tree):null;return `<div class="logic-chain-stage" data-logic-chain-pan-scroll="true" data-canvas-mouse-drag-pan="true" data-node-selection-preserves-viewport="true" onmousedown="startLogicChainPan(event)"><div class="logic-chain-surface" style="transform:translate(${panX}px,${panY}px) scale(${zoom});transform-origin:0 0;">${layout?logicChainMindMap(layout,nodes):empty('当前节点暂无可解析逻辑链。')}</div><div class="logic-chain-minimap" data-logic-chain-minimap="true">${logicChainMinimap(graph)}</div></div>`;}
                function logicChainBuildTree(graph,nodes){const segments=graph?.segments||[], segmentByChannel={};segments.forEach(seg=>{if(seg?.channel&&!segmentByChannel[seg.channel])segmentByChannel[seg.channel]=seg;});const edgeByFrom={};(graph?.edges||[]).forEach(edge=>{if(!edge?.from)return;(edgeByFrom[edge.from]||(edgeByFrom[edge.from]=[])).push(edge);});const focus=logicChainFocusNode(graph,nodes)||graph?.root||Object.values(nodes||{})[0];if(!focus)return null;return logicChainTreeFromNode(focus,graph,nodes,segmentByChannel,edgeByFrom,new Set(),0);}
                function logicChainFocusNode(graph,nodes){const meta=graph?.metadata||{}, rootRef=String(meta.rootRef||meta.rootChannel||graph?.root?.refId||graph?.root?.channel||''), rootType=String(meta.rootType||graph?.root?.refType||graph?.root?.type||'');const values=Object.values(nodes||{});if(!rootRef)return graph?.root||values[0];const aliases={device:['device','signal_device','virtual_block_device'],listener:['listener','signal_listener'],receiver:['receiver','signal_receiver'],relay:['relay','action_relay'],region_controller:['region_controller','region']};const types=aliases[rootType]||[rootType];return values.find(n=>types.includes(String(n.refType||n.type||''))&&String(n.refId||n.channel||'')===rootRef)||values.find(n=>String(n.channel||'')===rootRef&&String(n.type||'').toLowerCase()==='channel')||graph?.root||values[0];}
                function logicChainTreeFromNode(node,graph,nodes,segmentByChannel,edgeByFrom,visitedChannels,depth){if(!node)return null;const type=String(node.type||'').toLowerCase(), channel=node.channel||node.refId||'';const key=type==='channel'||type==='downstream_channel'?`channel:${channel}`:`node:${node.id}`;if((type==='channel'||type==='downstream_channel')&&visitedChannels.has(channel)){return {node:{...node,subtitle:'已在其它位置展开 / 循环引用'},children:[],cycle:true,depth};}const nextVisited=new Set(visitedChannels);if((type==='channel'||type==='downstream_channel')&&channel)nextVisited.add(channel);let childNodes=[];if(type==='channel'){const seg=segmentByChannel[channel]||{};childNodes=(seg.consumers||[]).map(id=>nodes[id]).filter(Boolean);}else if(type==='downstream_channel'){const seg=segmentByChannel[channel]||{};childNodes=(seg.consumers||[]).map(id=>nodes[id]).filter(Boolean);}else{childNodes=(edgeByFrom[node.id]||[]).map(edge=>nodes[edge.to]).filter(Boolean);}childNodes=logicChainSortTreeChildren(node,childNodes);const children=childNodes.map(child=>logicChainTreeFromNode(child,graph,nodes,segmentByChannel,edgeByFrom,nextVisited,depth+1)).filter(Boolean);return {node,children,cycle:false,depth};}
                function logicChainSortTreeChildren(parent,children){return (children||[]).slice().sort((a,b)=>{const at=String(a.type||''), bt=String(b.type||'');if(at===bt&&at.toLowerCase()==='action')return Number(a.metadata?.actionIndex??0)-Number(b.metadata?.actionIndex??0);return logicChainTypeRank(at)-logicChainTypeRank(bt)||String(a.label||a.id).localeCompare(String(b.label||b.id));});}
                function logicChainTypeRank(type){const t=String(type||'').toLowerCase();if(t==='producer')return 1;if(t==='channel'||t==='downstream_channel')return 2;if(t==='consumer')return 3;if(t==='action')return 4;return 9;}
                function logicChainLayoutTree(tree){const NODE_W=268,NODE_H=84,COL_GAP=156,ROW_GAP=42,MARGIN=44;function measure(item){const node=item.node||{}, key=node.id||node.channel||'', rawKids=item.children||[], collapsed=!!appState.logicChainCanvas.collapsedChannels?.[key];const kids=collapsed?[]:rawKids.map(measure);const kidsHeight=kids.length?kids.reduce((sum,k)=>sum+k.subtreeHeight,0)+ROW_GAP*(kids.length-1):0;return {...item,children:kids,childCount:rawKids.length,collapsed,subtreeHeight:Math.max(NODE_H,kidsHeight||NODE_H)};}function place(item,depth,top,flat,edges){const kids=item.children||[], kidsHeight=kids.length?kids.reduce((sum,k)=>sum+k.subtreeHeight,0)+ROW_GAP*(kids.length-1):0;item.x=MARGIN+depth*(NODE_W+COL_GAP);item.y=top+item.subtreeHeight/2-NODE_H/2;item.w=NODE_W;item.h=NODE_H;flat.push(item);let childTop=top+(item.subtreeHeight-kidsHeight)/2;kids.forEach(child=>{place(child,depth+1,childTop,flat,edges);edges.push({from:item,to:child,cycle:child.cycle});childTop+=child.subtreeHeight+ROW_GAP;});return item;}const root=measure(tree), flat=[], edges=[];place(root,0,MARGIN,flat,edges);const width=Math.max(760,...flat.map(n=>n.x+n.w+MARGIN)), height=Math.max(420,...flat.map(n=>n.y+n.h+MARGIN));return {root,flat,edges,width,height,nodeWidth:NODE_W,nodeHeight:NODE_H};}
                function logicChainMindMap(layout,nodes){const edgePaths=layout.edges.map(edge=>logicChainEdgePath(edge)).join('');const nodeHtml=layout.flat.map(item=>logicChainPositionedNode(item,nodes)).join('');return `<section class="logic-chain-mind-map" data-logic-chain-mind-map-tree="true" data-curved-continuous-connectors="true" data-parent-y-centered-by-child-subtree="true" data-local-badges-anchored="true" style="width:${layout.width}px;height:${layout.height}px"><svg class="logic-chain-edge-layer" width="${layout.width}" height="${layout.height}" viewBox="0 0 ${layout.width} ${layout.height}" aria-hidden="true"><defs><marker id="logic-chain-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z"></path></marker></defs>${edgePaths}</svg>${nodeHtml}</section>`;}
                function logicChainEdgePath(edge){const sx=edge.from.x+edge.from.w, sy=edge.from.y+edge.from.h/2, ex=edge.to.x, ey=edge.to.y+edge.to.h/2, dx=Math.max(80,ex-sx), c1=sx+dx*.48, c2=ex-dx*.42, cls=edge.cycle?' reference':'';return `<path class="logic-chain-edge${cls}" d="M ${sx} ${sy} C ${c1} ${sy}, ${c2} ${ey}, ${ex} ${ey}" marker-end="url(#logic-chain-arrow)"></path>`;}
                function logicChainPositionedNode(item,nodes){const node=item.node||{}, children=item.children||[], key=node.id||node.channel||'', type=String(node.type||'').toLowerCase(), isChannel=type==='channel'||type==='downstream_channel', collapsed=!!item.collapsed, childCount=Number(item.childCount||children.length||0);const badge=childCount?`<span class="logic-chain-node-badge">${collapsed?'已折叠':(isChannel?'并列消费者':'子节点')} ${childCount}</span>`:'';const toggle=(isChannel&&childCount)?`<button type="button" class="logic-chain-collapse" onclick='toggleLogicChainSubtree(${jsString(key)});event.stopPropagation();'>${collapsed?'展开下游':'折叠下游'}</button>`:'';const cycle=item.cycle?'<span class="logic-chain-node-badge warning">已在其它位置展开 / 循环引用</span>':'';return `<div class="logic-chain-tree-node depth-${Math.min(6,item.depth||0)}" data-tree-depth="${esc(item.depth||0)}" data-channel-subtree="${esc(node.channel||'')}" data-read-only-runtime-graph="true" style="left:${item.x}px;top:${item.y}px;width:${item.w}px;height:${item.h}px">${logicChainNodeCard(node,nodes,node.type==='downstream_channel'||item.cycle)}${badge}${toggle}${cycle}</div>`;}
                function logicChainNodeCard(node,nodes,downstream=false){if(!node)return '';const selected=appState.logicChainCanvas.selectedNodeId===node.id, disabled=node.enabled===false, type=String(node.type||'').toLowerCase(), order=type==='action'&&node.metadata?.actionIndex!=null?`#${Number(node.metadata.actionIndex)+1} `:'';return `<button type="button" class="logic-chain-node-card ${esc(type)} ${downstream?'downstream':''} ${disabled?'disabled':''} ${selected?'selected':''}" onclick='selectLogicChainNode(${jsString(node.id)})' title="${esc(node.subtitle||node.label||node.id)}" data-node-type="${esc(node.type||'')}" data-read-only-node="true"><span class="device-icon">${icon(logicChainNodeIcon(node))}</span><span><strong>${esc(order+(node.label||node.id))}</strong><small>${esc(node.subtitle||logicChainNodeTypeLabel(node.type,node.refType))}</small><small>${esc(node.channel||'')}</small></span></button>`;}
                ...
                function logicChainSelectedNodePanel(graph,nodes){const node=nodes[appState.logicChainCanvas.selectedNodeId]||graph?.root||{};const upstream=(graph?.edges||[]).filter(e=>e.to===node.id).map(e=>nodes[e.from]?.label||e.from);const downstream=(graph?.edges||[]).filter(e=>e.from===node.id).map(e=>nodes[e.to]?.label||e.to);return `<h2>节点信息</h2>${node.id?`<div class="logic-chain-node-inspector"><span class="device-icon">${icon(logicChainNodeIcon(node))}</span><h3>${esc(node.label||node.id)}</h3><p>${esc(logicChainNodeTypeLabel(node.type,node.refType))} · ${esc(node.refType||'')}</p>${pill(node.doctorStatus||node.status||'UNKNOWN')}<div class="identity-grid"><span class="k">ID</span><span class="v">${esc(node.refId||node.id)}</span><span class="k">频道</span><span class="v">${esc(node.channel||'-')}</span><span class="k">启用</span><span class="v">${node.enabled===false?'否':'是'}</span><span class="k">最近事件</span><span class="v">${fmtTime(node.lastEvent)}</span><span class="k">上游</span><span class="v">${upstream.length?upstream.map(esc).join(' / '):'暂无'}</span><span class="k">下游</span><span class="v">${downstream.length?downstream.map(esc).join(' / '):'暂无'}</span></div><div class="wa-quick-grid">${node.detailRoute?waButton('查看节点详情','eye',navigationAttr(node.detailRoute),'ghost'):waButton('查看节点详情','eye','disabled','ghost')}${node.channel?waButton('查看频道详情','active-channel',navigationAttr(signalHash(node.channel)),'ghost'):waButton('查看频道详情','active-channel','disabled','ghost')}${node.channel?waButton('查看事件历史','history',navigationAttr(historyHash(node.channel)),'ghost'):waButton('查看事件历史','history','disabled','ghost')}${waButton('诊断分析','doctor-overview',navigationAttr('#/doctor'),'ghost')}</div></div>`:empty('请选择画布中的节点。')}`;}
                function logicChainStats(stats){const items=[['频道',stats?.channelCount||0],['触发源',stats?.producerCount||0],['消费者',stats?.consumerCount||0],['动作',stats?.actionCount||0],['下游频道',stats?.downstreamChannelCount||0],['停用节点',stats?.disabledNodeCount||0]];return `<div class="logic-chain-stats">${items.map(([k,v])=>`<span><strong>${esc(v)}</strong>${esc(k)}</span>`).join('')}</div>`;}
                function logicChainVersionNote(){return `<div class="readonly-note" data-logic-chain-readonly-note="true">7.15 仅提供只读跨频道逻辑链查看器；WebAdmin-only metadata 不影响 runtime，条件分支留到 8.x ConditionEngine。</div>`;}
                function selectLogicChainNode(id){appState.logicChainCanvas.selectedNodeId=id||'';if(appState.currentLogicChainGraph)renderLogicChainViewer(appState.currentLogicChainGraph,appState.logicChainCanvas.routeInfo||{fallback:'#/logic-chains'}, {silent:true});}
