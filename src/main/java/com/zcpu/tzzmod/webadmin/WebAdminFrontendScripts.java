...
public final class WebAdminFrontendScripts {
    ...
    public static String appJs() {
        return new StringBuilder()
                ...
                function isDetailHash(hash){const h=String(hash||'');return h.startsWith('#/devices/')||h.startsWith('#/signals/')||h.startsWith('#/listeners/')||h.startsWith('#/signal-listeners/')||h.startsWith('#/logic-chains/')||h.startsWith('#/regions/')||h.startsWith('#/region-controllers/')||h.startsWith('#/actions/');}
                function isValidReturnHash(hash){const h=String(hash||'');if(!h.startsWith('#/'))return false;if(h.startsWith('#/login'))return false;if(h.includes('://'))return false;return ['#/dashboard','#/devices','#/virtual-block-devices','#/block-devices','#/receivers','#/listeners','#/signal-listeners','#/signals','#/signalbridge','#/logic-chains','#/doctor','#/diagnostics','#/signal-doctor','#/history','#/events','#/users','#/permissions','#/users-permissions','#/settings','#/system-settings','#/config','#/config-management','#/settings/config','#/regions','#/region-list','#/region-controllers','#/regionctl','#/actions','#/action-templates','#/templates'].some(prefix=>h===prefix||h.startsWith(prefix+'/')||h.startsWith(prefix+'?'));}
                ...
                const REALTIME_KNOWN_ROUTE_KEYS=['dashboard','signals','logicChains','receivers','listeners','actions','actionTemplates','devices','virtualBlockDevices','history','doctor','regions','regionControllers','users','settings','config'];
                ...
                function closeWebAdminModal(runHandler=true,force=false){
                  ...
                  if(runHandler&&handler){appState.modalCloseHandler=null;appState.modalClosePromise=Promise.resolve(handler()).then(result=>{if(result===false)appState.modalCloseHandler=handler;return result;}).finally(()=>{appState.modalClosePromise=null;});return appState.modalClosePromise;}
