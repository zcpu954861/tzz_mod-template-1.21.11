...
final class WebAdminLogicChainDiffScripts {
    ...
    static String appJs() {
        return new StringBuilder().append("""
                ...
                """)
                .append("""
                function logicChainDraftDiffRowsForSession(e){const rows=[];(e?.nodes||[]).filter(n=>String(n.type||'').toLowerCase()!=='channel_endpoint').forEach(n=>rows.push({title:logicChainDraftNodeTitle(n),field:'新增节点',before:'无',after:logicChainNodeTypeLabel(n.type,n.type),kind:'field'}));(e?.edges||[]).forEach(edge=>rows.push({title:'新增连线',field:edge.type||'edge',before:edge.from,after:edge.to,kind:'connection'}));(e?.draftChannels||[]).filter(c=>c.metadataDraft||c.cardDraft).forEach(c=>rows.push({title:c.displayName||c.channel,field:'频道端点',before:'无',after:c.channel,kind:'connection'}));if(e?.actionAppend?.confirmed){const a=e.actionAppend.action||{};rows.push({title:logicChainActionAppendOwnerLabel(e.actionAppend),field:'追加 Action',before:'无',after:`${labelActionType(a.type)} ${logicChainActionDraftSummary(a)}`,kind:'action'});}logicChainDraftDiffRowsForNestedActions(e).forEach(row=>rows.push(row));logicChainExistingNodeDraftsForPreview(e).forEach(d=>logicChainExistingDiffRows(d).forEach(row=>rows.push({...row,title:d.displayName||d.label||d.targetId||d.channel||d.id||'已有节点'})));logicChainActionDraftsForPreview(e).forEach(d=>{const title=`${logicChainActionAppendOwnerLabel(d)} #${Number(d.actionIndex||0)+1}`, fieldRows=logicChainDiffRows(d.original||{},logicChainComparableAction(d.action||{}),'action');if(fieldRows.length)rows.push({title,field:'Action 摘要',before:typedActionDisplaySummary(d.original||{},{redactCommand:true}),after:typedActionDisplaySummary(d.action||{},{redactCommand:true}),kind:'action'});fieldRows.forEach(row=>rows.push({...row,title}));if(d.operation==='disable'&&(d.original||{}).enabled!==false)rows.push({title,field:'operation',before:'replace',after:'disable',kind:'action'});});logicChainConfirmedNodeDeleteDrafts(e).forEach(d=>rows.push({title:d.ownerLabel||d.targetId||'已有节点',field:'删除节点',before:d.nodeType||'typed-owned',after:'保存时删除',kind:'action'}));logicChainConfirmedActionDeleteDrafts(e).forEach(d=>{const summary=d.actionSummary||typedActionDisplaySummary(d.action||d.original||{},{redactCommand:true});rows.push({title:`${logicChainActionAppendOwnerLabel(d)} #${Number(d.actionIndex||0)+1}`,field:'删除 Action',before:summary||'保留',after:'保存时删除',kind:'action'});});logicChainConfirmedActionReorderDrafts(e).forEach(d=>rows.push({title:logicChainActionAppendOwnerLabel(d),field:'重排 Action',before:`#${Number(d.fromIndex||0)+1}`,after:`#${Number(d.toIndex||0)+1}`,kind:'action'}));return rows;}
                """)
                .append("""
